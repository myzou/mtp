     ...
     <div class="x-body"> 
      ...
      <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so">
            <input type="text"    name="role_name" id="role_name"   placeholder="角色名称" autocomplete="off" class="layui-input">
...
<script type="text/html" id="barDemo">
    ...
    layui.use([ 'jquery','form', 'layedit','layer','laydate','table'], function(){
       ...
       table.render({
    	    elem: '#tableList'
    	    ,url:'${pageContext.request.contextPath }/role_sysRoleList.action'
    	    ,toolbar: '#toolbarDemo'
...
// 			  {type: 'checkbox', fixed: 'left'},
				{field:'role_id',  title: '角色id',align: 'center'  /* ,width: 80  */ }
    	       ,{field:'role_name',  title: '角色名称',align: 'center'  /* ,width: 80  */ }
    	       ,{field:'create_from',  title: '创建者',align: 'center'  /*  ,width:80 */  }
...
//     	            alert(JSON.stringify(data));
    	            if(obj.event === 'edit'){
    	          var content="./role_edit.jsp?role_id="+data.role_id;
//     	            layer.alert(JSON.stringify(cases));
     			var ind=     	 layer.open({
     			 ...
     			 layui.form.render('select'); 	
    	            	 ...
    	            	 $.ajax({
   	            		  url:"${pageContext.request.contextPath}/role_checkRoleMenuByRoleId.action",
   	            		  type:"post",
   	            		  data:"role_id="+data.role_id,
   	            		  dataType:"json",
   	            		  success:function(result){
   	            				  layer.msg('该角色关联了菜单，不能删除',{time:1000});
   	            				  return false ;
   	            		  } 
   	            	 	 });
    	            	 
    	            	 $.ajax({
      	            		  url:"${pageContext.request.contextPath}/role_checkUserRoleByRoleId.action",
      	            		  type:"post",
      	            		  data:"role_id="+data.role_id,
      	            		  dataType:"json",
      	            		  success:function(result){
      	            				  layer.msg('该该角色已经有用户使用，不能删除',{time:1000});
      	            				  return  false;
      	            		  } 
      	            	 	 });
    	            
    	            	
    	              layer.confirm('删除id为('+data.role_id+ ')的角色吗?', function(index){
    	            	  $.ajax({
    	            		  url:"${pageContext.request.contextPath}/role_delSysRole.action",
    	            		  type:"post",
    	            		  data:"role_id="+data.role_id,
    	            		  dataType:"json",
    	            		  success:function(result){
//     	            			  alert(result);
   	            				  layer.msg('删除成功<br/>'+data.role_id,{icon:6,time:300},function(){
	   	               	             obj.del();
	                	             layer.close(index);
