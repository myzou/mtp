		...
		<div class="layui-form-item">  
            <label for="intoPersonnel" class="layui-form-label" >
                  <span class="x-red">*</span>相关人员：
                   ...
                   <div class="layui-input-block w66">
                  <textarea placeholder="申请相关人员信息：" id="into_personnel" name="into_personnel" class="layui-textarea"		lay-verify="required|intoPersonnel" ></textarea>
              </div>
         	 </div>
            ...
            <label for="carryFacility" class="layui-form-label">
                ...
                <div class="layui-input-block w66">
                 <textarea id="carry_facility" name="carry_facility"   class="layui-textarea"	 autocomplete="off"	 lay-verify="required|carryFacility"
...
<!-- 					  <button type="button" class="layui-btn" id="back">返回</button> -->
	    ...
	    <div class="layui-input-block">
	      <button id ="notAdd" class="layui-btn" lay-submit=""   type="button" lay-filter="add" >立即提交</button>
		    			  ...
		    			  btn: ['确定','取消'] //按钮
			    			...
			    			$.ajax({
								...
								success:function(result){
									$("#submit_flag").val("true");
									  layer.msg("增加成功,已经发送邮件", {icon: 6,time:500},function () {
					 		                // 获得frame索引
			    			...
			    			$.ajax({
								...
								success:function(result){
								$("#submit_flag").val("true");
							      layer.msg("增加成功，已经发送邮件", {icon: 6},function () {
				 		                // 获得frame索引
				 		                var index = parent.layer.getFrameIndex(window.name);
				 		                //关闭当前frame
				 		                parent.layer.close(index);
...
//			layer.alert(JSON.stringify(result[0]));
				...
				$("#name").val(email.name);
	        	 ...
	        	 $("#into_personnel").val(email.into_personnel);
	        	 $("#carry_facility").val(email.carry_facility);
	        	 if(email.au_status=='D'){
	        		 $("#notAdd").addClass("layui-btn-disabled");
	        		 $("#notAdd").attr('disabled',true);
	        		 $("#notAdd").attr('title','该单已经删除，无法编辑发送');
	        	 }
// 	        	 拼接人员
